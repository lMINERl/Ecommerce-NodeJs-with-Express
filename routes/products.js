...
router.get('/', authenticationMiddleware, async (req, res, next) => {
  ...
  const items = item.map(p => {
    return { _id: p.id, name: p.name, price: p.price, discount: p.discount, isOnSale: p.isOnSale };
  });
...
router.get('/:_id', authenticationMiddleware, async (req, res, next) => {
  const item = await productModel.findById(req.params._id).catch(err => createError(400, err));
  res.send(item);
});
...
router.delete('/:_id', authenticationMiddleware, async (req, res, next) => {
  ...
  const item = await productModel.deleteOne({ _id: req.params._id }).catch(err => createError(400, err));
  res.send(item);
});
